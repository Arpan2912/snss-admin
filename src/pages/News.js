...
export default function News(props) {
  ...
  const getAllNews = async () => {
    const blogsResponse = await getBlogs('news_and_update');
  ...
  const openBlog = (blog) => {
    if (blog) {
      ...
      navigate('/add-blog', { state: { defaultType: 'news_and_update' } })
    }
  }
  ...
  const copyToClipboard = (url) => {
    const textToCopy = `https://www.snssindia.in/api/news/${url}`
    alert(textToCopy);
  }
      ...
      {blogs.length === 0 && (
        <Row>
          <Col className="text-center py-4 text-muted">
            No news items found. Click "Add" to create a new news item.
          </Col>
        </Row>
      )}
    </div>)
